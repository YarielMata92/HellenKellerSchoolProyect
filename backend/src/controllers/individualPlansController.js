...
export const getIndividualPlans = async (req, res)=>{
    try {
        const result = await pool.query("SELECT ip.id AS id, st.id AS student_id, st.name AS student_name, st.lastname AS student_lastname, us.id AS teacher_id, us.name AS teacher_name, us.lastname AS teacher_lastname, ip.goal AS goal, pr.progress AS progress, pr.id AS report_id FROM individualized_plans ip JOIN students st ON ip.student_id = st.id JOIN users us ON ip.teacher_id = us.id JOIN LATERAL (SELECT pr.* FROM progress_reports pr WHERE pr.plan_id = ip.id ORDER BY pr.id DESC LIMIT 1) pr ON TRUE WHERE ip.visible = true;")
