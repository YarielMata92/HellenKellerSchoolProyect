...
import { deleteIndividualPlan, getIndividualPlan, getTeacherIndividualPlan } from "../../services/IndividualPlan.service"
...
export default function IndividualPlansTable() {
  ...
  const fetchDataAsTeacher = async () => {
    try {
      const { data } = await getTeacherIndividualPlan(id);
      setTableData(data)
      console.log(tableData)
    } catch (err) {
      console.log(err)
